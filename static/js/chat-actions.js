...
<body>
  ...
  function renderArtifactCard(artifact, containerEl) {
    ...
    if (artifactId && artifactList.querySelector(`[data-artifact-id="${artifactId}"]`)) {
      return;
    }
    ...
    nameSpan.textContent = artifact.title && artifact.title !== artifact.filename ? `${artifact.filename} (${artifact.title})` : artifact.filename;

    const sizeSpan = document.createElement('span');
    sizeSpan.className = 'artifact-size';
    sizeSpan.textContent = window.CloudGPTAttachments && artifact.size ? window.CloudGPTAttachments.formatFileSize(artifact.size) : `${artifact.size || 0} B`;

    infoDiv.append(nameSpan, sizeSpan);

    const downloadBtn = document.createElement('a');
    downloadBtn.className = 'artifact-download-btn';
    downloadBtn.href = `/api/artifacts/${encodeURIComponent(artifactId)}/download`;
  ...
  function renderBundleDownloadBtn(zipUrl, count, containerEl) {
    if (!zipUrl || !containerEl) return;
    let artifactList = containerEl.querySelector('.artifact-list');
    if (!artifactList) {
      artifactList = document.createElement('div');
      artifactList.className = 'artifact-list';
    ...
    if (artifactList.querySelector('.artifact-bundle-btn')) return;

    const bundleBtn = document.createElement('a');
    bundleBtn.className = 'artifact-bundle-btn';
    bundleBtn.href = zipUrl;
    bundleBtn.innerHTML = `
      <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <span>Download Full Architecture Project (${count} files .ZIP)</span>
    `;
    artifactList.append(bundleBtn);
  }
